...
/**
 * AI 프롬프트 관리 API (관리자 전용).
 *  GET    /api/admin/prompts            전체 프롬프트 + 현재 상태
 *  PUT    /api/admin/prompts/{key}      오버라이드 저장 (검증 후)
 *  POST   /api/admin/prompts/{key}/reset 기본값으로 되돌리기
 */
@RestController
...
public class AdminPromptController {
    ...
    public ResponseEntity<?> list(Authentication authentication) {
        ...
        try {
            List<AiPromptResponse> result = aiPromptService.getAdminViews().stream()
    ...
    public ResponseEntity<?> update(
        ...
        try {
            AiPromptService.PromptAdminView view =
                    aiPromptService.saveOverride(key, request.getContent(), authentication.getName());
            log.info("[ADMIN/PROMPTS] user={}(ADMIN), SAVE key={} customized={}",
                    authentication.getName(), key, view.customized());
