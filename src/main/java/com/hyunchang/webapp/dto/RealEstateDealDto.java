...
/**
 * 아파트 실거래 1건 (국토교통부 실거래가 공개 API 기반).
 * 거래유형(dealType)에 따라 금액 필드 사용이 달라진다:
 *   SALE    : dealAmount(거래금액) 사용
 *   JEONSE  : deposit(보증금) 사용, monthlyRent = 0
 *   MONTHLY : deposit(보증금) + monthlyRent(월세) 사용
 * 모든 금액 단위는 만원(국토부 API 원본 단위 유지).
 */
@Data
@Builder
@NoArgsConstructor
