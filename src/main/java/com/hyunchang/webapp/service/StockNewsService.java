...
/**
 * 주식 뉴스 서비스. - 국내/해외 RSS 피드 수집 및 파싱 - 해외 뉴스 한글 자동 번역 (Google Translate 비공식 API) - 20분 인메모리 캐시 (동시성:
 * synchronized 블록으로 중복 fetch 방지)
 */
@Service
public class StockNewsService {
    ...
    private static final Set<String> KR_STOCK_KEYWORDS =
            Set.of(
                    // 시장/지수
                    "주가",
                    "주식",
    ...
    private static final Set<String> US_STOCK_KEYWORDS =
            Set.of(
                    // 핵심 주식 용어
                    "stock",
                    "stocks",
    ...
    private static final Set<String> KR_EXCLUDE_TITLE =
            Set.of(
                    "헤드라인", "뉴스브리핑", "뉴스 브리핑", "오늘의 뉴스", "이 시각 뉴스", "주요 뉴스", "뉴스 요약", "아침 뉴스",
                    "저녁 뉴스");
    ...
    private static final List<String[]> RSS_SOURCES =
            List.of(
                    // ── 국내 (KR) ────────────────────────────────────────────────
                    ...
                    new String[] {
                        "Google News KR 증시",
                        googleNewsRss("증시 OR 코스피 OR 코스닥 OR 주가 OR 실적", "ko", "KR", "ko-KR"),
                        "KR"
                    },
                    new String[] {
                        "Google News KR 경제지",
                        googleNewsRss(
                                "주식 OR 투자 OR 증권 site:hankyung.com OR site:mk.co.kr OR site:sedaily.com OR site:edaily.co.kr OR site:news.mt.co.kr",
                                "ko",
                                "KR",
                                "ko-KR"),
                        "KR"
                    ...
                    // ── 해외 (US) ────────────────────────────────────────────────
                    new String[] {
                        "Yahoo Finance", "https://finance.yahoo.com/rss/topfinstories", "US"
                    },
                    new String[] {
                    ...
                    new String[] {
                        ...
                        googleNewsRss(
                                "stocks OR earnings OR \"S&P 500\" OR Nasdaq", "en", "US", "en-US"),
                        "US"
                    },
                    new String[] {
                        "Google News US Financial Press",
                        googleNewsRss(
                                "stock OR earnings site:reuters.com OR site:cnbc.com OR site:marketwatch.com OR site:barrons.com OR site:fool.com",
                                "en",
                                "US",
                                "en-US"),
                        "US"
    ...
    public StockNewsService(
            RestTemplate restTemplate, ObjectMapper objectMapper, RssClient rssClient) {
    ...
    /**
     * 시장별 뉴스 조회. market: "KR" | "US" | 그 외(전체). 캐시 TTL(20분) 이내면 캐시 반환, 만료 시 RSS 재수집. #4:
     * synchronized 블록으로 동시 요청 시 중복 fetch 방지.
     */
    public List<StockNewsDto> getNews(String market, boolean force) {
        ...
        if ("US".equalsIgnoreCase(market)) {
            synchronized (usLock) {
                ...
                List<CompletableFuture<StockNewsDto>> futures =
                        fresh.stream()
                                .map(
                                        n ->
                                                CompletableFuture.supplyAsync(
                                                        () -> {
                                                            ...
                                                            if (translatedDesc != null
                                                                    && translatedDesc.length()
                                                                            > 300) {
                                                                translatedDesc =
                                                                        translatedDesc.substring(
                                                                                        0, 300)
                                                                                + "…";
                                                            }
    ...
    private List<StockNewsDto> fetchByMarket(String market) {
        List<String[]> sources =
                RSS_SOURCES.stream().filter(src -> src[2].equalsIgnoreCase(market)).toList();

        List<Future<List<StockNewsDto>>> futures =
                sources.stream()
                        .map(
                                src ->
                                        SHARED_POOL.submit(
                                                () -> {
                                                    try {
                                                        return parseRss(src[1], src[0], src[2]);
                                                    } catch (Exception e) {
                                                        log.warn(
                                                                "RSS 파싱 실패 [{}]: {}",
                                                                src[0],
    ...
    private boolean isStockRelated(String title, String description, String market) {
        ...
        Set<String> keywords =
                "KR".equalsIgnoreCase(market) ? KR_STOCK_KEYWORDS : US_STOCK_KEYWORDS;
    ...
    private static String normalizeText(String text) {
        ...
        return text.replace('‘', '\'')
                ...
                .replace('—', '-') // 엔 대시 / 엠 대시
                .replace("&#8217;", "'")
                .replace("&#8216;", "'") // HTML 엔티티 단따옴표
                .replace("&#8220;", "\"")
                .replace("&#8221;", "\"") // HTML 엔티티 쌍따옴표
                .replace("&#8211;", "-")
                .replace("&#8212;", "-") // HTML 엔티티 대시
                .replace("&rsquo;", "'")
                .replace("&lsquo;", "'")
