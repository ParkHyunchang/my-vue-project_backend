...
public final class AiPromptCatalog {
    ...
    private static PromptDefinition portfolioIsaAnalysis() {
        return new PromptDefinition(
            ...
            - **현황**: 평단가 대비 손익률 N%, 일변동률 N%
            - **밸류에이션 & 적립 타이밍**: PER N배, PBR N배 — 현 구간이 [과매도/적정/과매수] 판단 (없으면 '데이터 없음')
            - **수익성·배당**: ROE N%, 배당수익률 N% (없으면 '데이터 없음')
            - **공시·뉴스**: 이 자산 관련 핵심 공시 또는 뉴스 1건 (없으면 '해당 없음')
            - **적립 판단**: PER/PBR 수준과 뉴스를 근거로 지금이 추가적립 타이밍인지 1~2문장. 구체적 수치 인용 필수.

            ## 추가 적립 우선순위
            근거(PER/PBR 저평가·호재 공시)가 확인된 자산 1~3개, 가장 시급한 순. 뚜렷한 근거 없으면 '해당 없음'.
    ...
    private static PromptDefinition portfolioIrpAnalysis() {
        return new PromptDefinition(
            ...
            - **현황**: 평단가 대비 손익률 N%, 일변동률 N%
            - **안정성 지표**: PBR N배, ROE N%, 배당수익률 N% — 은퇴자산 관점 적합 여부 (없으면 '데이터 없음')
            - **이익의 질**: CFO/Net Income N.N (없으면 '데이터 없음'; 1.0 미만이면 "이익의 질 점검 필요" 표시)
            - **공시·뉴스**: 이 자산 관련 핵심 공시 또는 뉴스 1건 (없으면 '해당 없음')
            - **은퇴자산 판단**: 변동성·안정성·배당 중심 판단 1~2문장. 구체적 수치 인용 필수.
