 ...
 *
 * <p>프롬프트는 세 부분으로 구성된다: 1. instruction (편집 가능) — 페르소나 + "어떻게 분석/답변할지" 지침. 관리자가 수정하는 유일한 부분. 2.
 * fixedContext (고정) — 분석에 들어가는 데이터 영역. {{변수}} 를 포함하며 코드가 값을 채운다. 3. fixedSchema (고정) — 응답 형식(JSON
 * 스키마). 사람이 건드리면 파싱이 깨지므로 고정.
 *
 * <p>최종 프롬프트 = instruction + fixedContext(값 치환) + fixedSchema.
 */
