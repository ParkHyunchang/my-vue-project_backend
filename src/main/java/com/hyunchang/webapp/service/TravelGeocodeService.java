 ...
 *
 * <p>브라우저에서 Nominatim을 직접 호출하지 않고 백엔드가 대신 호출한다: - Nominatim 이용정책이 요구하는 식별 가능한 User-Agent를 서버에서 명시 -
 * 프론트는 same-origin(/api/travel/geocode) 호출 → CSP에 외부 도메인 개방 불필요 - 동일 검색어는 짧게 캐시해 외부 호출 빈도를 낮춤(정책상
 * 권장)
 *
 * <p>응답 JSON(배열)을 그대로 통과시켜 프론트가 display_name/lat/lon을 사용한다.
 */
@Service
public class TravelGeocodeService {
    ...
    private static final String USER_AGENT =
            "HyunchangHome-Travel/1.0 (https://hyunchang.synology.me; gusckd1988@gmail.com)";
