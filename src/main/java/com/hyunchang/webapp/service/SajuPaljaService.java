 ...
 * 시두법(오둔시결)으로 로컬 계산한다. 전통 관례에 따라 23:00~23:59 출생(야자시)은 다음날 간지를 사용한다. 음력 생년월일이 입력되면 먼저
 * 양력으로 환산한 뒤 동일한 로직을 그대로 적용한다.
 */
@Service
public class SajuPaljaService {
    ...
    public Optional<SajuResultDto> compute(SajuBirthInputDto input) {
        ...
        if (input.isLunar()) {
            if (input.lunarYear() == null || input.lunarMonth() == null || input.lunarDay() == null) {
                return Optional.empty();
            }
            SajuCalendarClient.LunarResolution resolved =
                    calendarClient.resolveLunar(
                            input.lunarYear(), input.lunarMonth(), input.lunarDay(), input.leapMonth());
        ...
        SajuResultDto result =
                SajuResultDto.builder()
                        ...
                        .fiveElementCounts(countFiveElements(yearPillar, monthPillar, dayPillar, hourPillar))
                        .build();
        return Optional.of(result);
    }

    /**
     * 윤달 기간에는 KASI API가 월건(月建)을 빈 문자열로 돌려준다 — 윤달이 그 해의 어느 절기 구간에 속하는지 이 API로는 알 수 없기 때문이다.
     * 정확한 절기 경계 계산 없이는 해소할 수 없으므로, 가장 가까운 날짜(앞뒤 번갈아 최대 {@value
     * #MONTH_GANJI_FALLBACK_RANGE_DAYS}일)의 월건을 근사값으로 빌려온다. 절기 경계가 윤달 중간에 걸치면 실제와 다를 수 있다.
     */
