...
/**
 * 부동산 실거래가 오케스트레이션 서비스.
 * - 법정동 시군구 코드 검색 (resources/realestate/kr-lawd-codes.json)
 * - 거래유형(매매/전세/월세)별 실거래 검색 (RealEstateApiService 위임 + 최근 N개월 합산)
 */
@Service
public class RealEstateService {
    ...
    public List<RealEstateDealDto> search(String lawdCd, String dealType, int months) {
        ...
        for (int i = 0; i < safeMonths; i++) {
            ...
            List<RealEstateDealDto> monthDeals = isSale
                ? apiService.getTrades(lawdCd, ymd)
                : apiService.getRents(lawdCd, ymd);
     ...
     * @param dealType SALE | JEONSE | MONTHLY
     */
    public PropertyQuoteDto getQuote(String lawdCd, String dealType, String aptName, Double areaM2) {
        ...
        List<RealEstateDealDto> matched = deals.stream()
            .filter(d -> {
                String dn = normalize(d.getAptName());
                boolean nameMatch = dn.equals(target) || dn.contains(target) || target.contains(dn);
                boolean areaMatch = areaM2 == null || areaM2 <= 0
        ...
        long avg = Math.round(matched.stream()
