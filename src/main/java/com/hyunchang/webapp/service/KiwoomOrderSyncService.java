...
public class KiwoomOrderSyncService {
    ...
    public KiwoomOrderSyncService(KiwoomTradeService trade, KiwoomTradeProposalRepository proposals, KiwoomStrategyAuditService audit, KiwoomProperties props) { this.trade = trade; this.proposals = proposals; this.audit = audit; this.props = props; }
    ...
    private boolean apply(JsonNode record) {
        ...
        return proposals.findByBrokerOrderNo(orderNo).map(proposal -> {
            int ordered = number(record, "ord_qty", "order_qty", "qty");
            int filled = number(record, "cntr_qty", "filled_qty", "exec_qty");
            int remaining = number(record, "rmn_qty", "unfilled_qty", "ord_remain_qty");
            if (filled == 0 && ordered > 0 && remaining >= 0) filled = Math.max(0, ordered - remaining);
            if (remaining == 0 && filled == 0) return false;
            Long price = longValue(record, "cntr_prc", "avg_cntr_prc", "filled_price");
            KiwoomTradeProposal.Status before = proposal.getStatus();
            proposal.syncFill(filled, remaining, price); proposals.save(proposal);
            if (before != proposal.getStatus()) audit.log("ORDER_STATUS_SYNC", proposal.getId(), "키움 주문 상태가 " + proposal.getStatus() + "로 변경되었습니다.");
            return true;
        }).orElse(false);
    }
    ...
    private Long longValue(JsonNode node, String... fields) { for (String field : fields) if (node.hasNonNull(field)) { String raw = node.path(field).asText().replace(",", "").trim(); try { return Long.parseLong(raw); } catch (NumberFormatException ignored) {} } return null; }
    private String trim(String value) { return value == null ? "unknown" : value.substring(0, Math.min(500, value.length())); }
