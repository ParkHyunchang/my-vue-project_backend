...
public class SubscriptionService {
    ...
    public Subscription create(Subscription input) {
        ...
        log.info("[SUBSCRIPTION] user={}({}), CREATE id={} name={} amount={} cycle={} next={}",
                SecurityUtils.getCurrentUserId(), SecurityUtils.getCurrentUserRoleName(),
                saved.getId(), saved.getName(), saved.getAmount(), saved.getBillingCycle(), saved.getNextBillingDate());
        return saved;
    }
    ...
    public Subscription update(Long id, Subscription input) {
        ...
        existing.setCurrency(input.getCurrency() == null || input.getCurrency().isBlank() ? "KRW" : input.getCurrency());
        ...
        existing.setStatus(input.getStatus() == null || input.getStatus().isBlank() ? "ACTIVE" : input.getStatus());
        existing.setColor(input.getColor());
        existing.setMemo(input.getMemo());
        Subscription saved = subscriptionRepository.save(existing);
        log.info("[SUBSCRIPTION] user={}({}), UPDATE id={} name={} amount={} cycle={} next={} status={}",
                SecurityUtils.getCurrentUserId(), SecurityUtils.getCurrentUserRoleName(),
                saved.getId(), saved.getName(), saved.getAmount(), saved.getBillingCycle(), saved.getNextBillingDate(), saved.getStatus());
        return saved;
    }
    ...
    public void delete(Long id) {
        ...
        log.info("[SUBSCRIPTION] user={}({}), DELETE id={} name={}",
                SecurityUtils.getCurrentUserId(), SecurityUtils.getCurrentUserRoleName(),
    ...
    private void validate(Subscription input) {
        ...
        if (input.getStartedAt() != null && input.getStartedAt().isAfter(LocalDate.now().plusYears(10))) {
            throw new RuntimeException("시작일이 비정상적입니다.");
        }
        if (input.getStatus() != null && !input.getStatus().isBlank() && !ALLOWED_STATUS.contains(input.getStatus())) {
