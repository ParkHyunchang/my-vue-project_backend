...
/**
 * Yahoo Finance API 연동 서비스. - 미국 시총 상위 종목 조회 (스크리너) - 개별 종목 시세 조회 (v8 chart) - 종목 검색 (영문) - 히트맵용 개별
 * 종목 조회 - 미국 종목 한글명/영문명 변환
 */
@Service
public class YahooFinanceService {
     ...
     * screener API와 마지막 성공 스냅샷이 모두 없을 때만 사용하는 최후 안전망이며, v7/quote 또는 v8/chart로 시총을 조회한 뒤 내림차순 정렬해
     * 상위를 추출합니다. shares 값은 Yahoo가 marketCap=0을 반환할 때만 가격×주식수 계산용 폴백으로 사용됩니다.
     */
    ...
    public List<String[]> getUsStocksFallback() {
        return usStocksFallback;
    }
        ...
        synchronized (this) {
            ...
            for (String sym : symbols) {
                ...
                String display =
                        (ko != null && !ko.isBlank())
                                ? ko
                                : (enFull != null && !enFull.isBlank()) ? enFull : sym;
    ...
    /**
     * 미국 종목 로컬 검색 — 한글명("테슬라"), 영문명("Tesla"/"tesla"), 티커("TSLA") 모두 매칭. us-names-ko.json +
     * us-en-names.json + us-stocks-fallback.json 의 합집합 인덱스에서 부분일치(contains) 검색을 수행합니다. Yahoo API에
     * 의존하지 않아 빠르고 안정적입니다.
     */
    ...
    /**
     * Yahoo Finance screener API로 미국 상장 시총 상위 종목을 동적으로 조회합니다. region=us 필터로 NYSE/NASDAQ 상장 종목(ADR
     * 포함)을 시가총액 내림차순 정렬해 반환합니다. 하드코딩 후보 풀이 없으므로 시총 변동/신규 상위 진입 종목까지 그대로 반영됩니다. 실패 시 빈 리스트 반환 — 호출자가
     * 폴백을 처리합니다.
     */
    ...
    private List<RawQuote> doFetchTopMarketCapUs(int count) {
        ...
        try {
            ...
            for (JsonNode q : quotes) {
                ...
                String name =
                        !longName.isEmpty() ? longName : !shortName.isEmpty() ? shortName : symbol;
                ...
                double shares =
                        firstPositiveNumber(
                                q.path("sharesOutstanding"), q.path("impliedSharesOutstanding"));
    ...
    /**
     * Yahoo Finance screener API 호출에 필요한 cookie + crumb 토큰을 갱신합니다. 30분 TTL 캐싱으로 매 요청마다 토큰 재발급 비용을
     * 피합니다.
     */
        ...
        synchronized (crumbLock) {
            ...
            try {
                ...
                try {
                    ...
                    setCookies =
                            (errHeaders != null) ? errHeaders.get(HttpHeaders.SET_COOKIE) : null;
                } catch (RestClientException ignored) {
                    // 네트워크 차단 등으로 fc.yahoo.com 자체가 실패한 경우 — finance.yahoo.com 폴백
                }
                if (setCookies == null || setCookies.isEmpty()) {
                    try {
                        ResponseEntity<String> mainResp =
                                restTemplate.exchange(
                        ...
                        setCookies =
                                e.getResponseHeaders() != null
                                        ? e.getResponseHeaders().get(HttpHeaders.SET_COOKIE)
                                        : null;
                    }
                }
                if (setCookies == null || setCookies.isEmpty()) {
                ...
                ResponseEntity<String> crumbResp =
                        restTemplate.exchange(
    ...
    /**
     * Yahoo Finance v7/quote API로 여러 종목을 한 번에 조회합니다. v8/chart 개별 호출 대비 시가총액 데이터가 정확하며 API 호출 횟수가
     * 1회로 줄어듭니다. 2025년부터 Yahoo가 v7/quote에도 cookie + crumb 인증을 요구하기 시작하여 인증 토큰을 갱신해 사용합니다. 실패 시 빈
     * 리스트를 반환하고 호출자가 폴백을 처리합니다.
     */
    public List<RawQuote> fetchBulkQuotes(List<String[]> stocks) {
        ...
        try {
            ...
            JsonNode results =
                    objectMapper.readTree(resp.getBody()).path("quoteResponse").path("result");
    ...
    /**
     * 종목의 재무 펀더멘털(PER·PBR·ROE·마진·성장률·컨센서스 등)을 조회한다. v10 quoteSummary 는 cookie+crumb 인증이 필요하므로 기존 인증
     * 토큰을 재사용한다. 실패 시 null 을 반환해 호출자가 '데이터 없음'으로 처리하게 한다.
     */
        ...
        try {
            ...
            String url =
                    "https://query2.finance.yahoo.com/v10/finance/quoteSummary/"
                            ...
                            + "?modules="
                            + URLEncoder.encode(modules, StandardCharsets.UTF_8)
                            + "&lang=en-US&region=US&crumb="
                            + URLEncoder.encode(cachedCrumb, StandardCharsets.UTF_8);
    ...
    /**
     * quoteSummary 가 404/차단되는 경우 v7/finance/quote 에서 가능한 재무 필드만 폴백 수집한다. v7 quote 는
     * 가격/시총/PER/PBR/배당수익률 등 제한된 필드만 제공하지만, AI 프롬프트에는 "재무 데이터 없음"보다 훨씬 유용하고 호출 안정성도 높다.
     */
    private JsonNode fetchFundamentalsFromQuote(String symbol, boolean useAuth) {
        ...
        try {
            ...
            putRaw(
                    summaryDetail,
                    "dividendYield",
                    firstPresent(quote, "dividendYield", "trailingAnnualDividendYield"));
            putRaw(
                    summaryDetail,
                    "priceToSalesTrailing12Months",
                    firstPresent(quote, "priceToSalesTrailing12Months", "priceToSales"));
    ...
    private List<String[]> loadFallbackList(String resourcePath) {
        try (InputStream is = getClass().getClassLoader().getResourceAsStream(resourcePath)) {
            ...
            List<Map<String, String>> list = objectMapper.readValue(is, new TypeReference<>() {});
            return list.stream()
                    .map(
                            m ->
                                    new String[] {
