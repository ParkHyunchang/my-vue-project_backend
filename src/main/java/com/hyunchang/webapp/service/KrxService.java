...
/** KRX(한국거래소) Open API 연동 서비스. - KOSPI/KOSDAQ 시총 순위 조회 - 국내 종목 한글명 조회 및 캐시 - 한글 검색어로 종목 검색 */
@Service
public class KrxService {
    ...
    public List<String[]> getKrStocksFallback() {
        return krStocksFallback;
    }

    public List<String[]> getKqStocksFallback() {
        return kqStocksFallback;
    }

    public Map<String, String> getKrSectorMap() {
        return krSectorMap;
    }

    public Map<String, String> getKrHeatmapNameFallback() {
        return krHeatmapNameFallback;
    }
    ...
    /** KOSPI 종목 목록 캐시 조회 (검색·히트맵 공용, 6시간 TTL). 네이버 금융에서 실시간으로 가져오며, 실패 시 이전 캐시 → 빈 리스트. */
    public List<String[]> getTopStocksCached(int count) {
        long now = System.currentTimeMillis();
        if (krxStocksCache == null || (now - krxStocksCacheTime) > CACHE_TTL_MS) {
            List<String[]> fresh =
                    naverService.getTopStocksKospiCached(50).stream()
    ...
    /** 한글 검색어로 KRX 종목 검색. krNameLookup (KRX OpenAPI 전체 + 네이버 상위 100 + 폴백)을 검색 풀로 사용합니다. */
    ...
    /**
     * KR 심볼(.KS/.KQ)의 한글명 반환. 없으면 null. 1) KRX 공식 OpenAPI 전체 사전(ISU_ABBRV, 2500+개) 2) 네이버 기반 상위
     * 100개 + 하드코딩 폴백 룩업 3) 히트맵용 하드코딩 폴백 4) 네이버 금융 개별 종목 페이지 스크래핑 (우선주·소형주 등 최후 폴백)
     */
    ...
    private void buildKrNameLookupIfNeeded() {
        if (krNameLookup != null && (System.currentTimeMillis() - krNameLookupTime) < CACHE_TTL_MS)
            return;
        buildKrNameLookup();
    }
    ...
    private synchronized void buildKrNameLookup() {
        ...
        krxApiService
                .getTopKospiStocks(Integer.MAX_VALUE)
                .forEach(s -> map.put(s.symbol().toUpperCase(), s.name()));
        krxApiService
                .getTopKosdaqStocks(Integer.MAX_VALUE)
                .forEach(s -> map.put(s.symbol().toUpperCase(), s.name()));
        ...
        naverService
                .getTopStocksKospiCached(50)
                .forEach(s -> map.putIfAbsent(s.symbol().toUpperCase(), s.name()));
        naverService
                .getTopStocksKosdaqCached(50)
                .forEach(s -> map.putIfAbsent(s.symbol().toUpperCase(), s.name()));
        ...
        log.info(
                "KR 종목명 룩업 맵 빌드 완료 (KRX주식+ETF/ETN+Naver+폴백): 총 {}개 (ETP {}개)",
                map.size(),
                krEtpSymbols.size());
    }
