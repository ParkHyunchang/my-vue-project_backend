...
import com.hyunchang.webapp.dto.LandAnalysisResponse;
import com.hyunchang.webapp.dto.LandDealDto;
import com.hyunchang.webapp.dto.LandStatsDto;
import com.hyunchang.webapp.dto.RealEstateAnalysisResponse;
import com.hyunchang.webapp.dto.RealEstateAnalysisResult;
import com.hyunchang.webapp.dto.RealEstateDealDto;
import com.hyunchang.webapp.dto.RealEstateNewsDto;
import com.hyunchang.webapp.dto.RealEstateStatsDto;
 ...
 *
 * <p>흐름: 1. 검색한 시군구+거래유형의 최근 실거래 수집 → 결정적 통계 집계(평균/최저/최고/추세/평형별) 2. 통계 + 대표 거래 + 부동산 뉴스로 프롬프트 조립 3.
 * AiProviderChain (Gemini → Groq → Cloudflare) 호출 4. JSON 파싱 → 응답 조립 (AI 실패해도 통계는 항상 반환)
 */
@Service
public class RealEstateAnalysisService {
    ...
    public RealEstateAnalysisService(
    ...
    public RealEstateAnalysisResponse analyze(String lawdCd, String dealType) {
        String dt =
                (dealType == null || dealType.isBlank())
                        ? "SALE"
                        : dealType.toUpperCase(Locale.ROOT);
        ...
        List<RealEstateDealDto> deals =
                SafeCalls.get(
                        () -> realEstateService.search(lawdCd, dt, ANALYSIS_MONTHS),
                        List.of(),
                        log,
                        "[AI/RealEstate] context collection failed");

        if (deals.isEmpty()) {
            return RealEstateAnalysisResponse.builder()
                    .noData(true)
                    .stats(
                            RealEstateStatsDto.builder()
                                    .sigungu(sigungu)
                                    .dealType(dt)
                                    .totalCount(0)
                                    .areaBuckets(List.of())
        ...
        List<RealEstateNewsDto> news =
                SafeCalls.get(
                        () -> realEstateNewsService.getNews(false),
                        List.of(),
                        log,
                        "[AI/RealEstate] context collection failed");
    ...
    public LandAnalysisResponse analyzeLand(String lawdCd) {
        ...
        List<LandDealDto> deals =
                SafeCalls.get(
                        () -> realEstateService.searchLand(lawdCd, ANALYSIS_MONTHS),
                        List.of(),
                        log,
                        "[AI/RealEstate] context collection failed");
        ...
        if (deals.isEmpty()) {
            return LandAnalysisResponse.builder()
                    .noData(true)
                    .stats(
                            LandStatsDto.builder()
                                    .sigungu(sigungu)
                                    .totalCount(0)
                                    .jimokBuckets(List.of())
                                    .zoneBuckets(List.of())
                                    .build())
                    .build();
        }
    ...
    private LandStatsDto computeLandStats(List<LandDealDto> deals, String sigungu) {
        long[] units =
                deals.stream().mapToLong(LandDealDto::getPricePerM2).filter(v -> v > 0).toArray();
    ...
    private double unitAvg(List<LandDealDto> ds) {
        return ds.stream()
                .mapToLong(LandDealDto::getPricePerM2)
                .filter(v -> v > 0)
                .average()
                .orElse(0);
    }
    ...
    private List<LandStatsDto.Bucket> bucketBy(
            List<LandDealDto> deals, Function<LandDealDto, String> key) {
        Map<String, List<LandDealDto>> grouped =
                deals.stream()
                        .filter(
                                d ->
                                        d.getPricePerM2() > 0
        ...
        return grouped.entrySet().stream()
                .map(
                        e ->
                                LandStatsDto.Bucket.builder()
                                        .label(e.getKey())
                                        .count(e.getValue().size())
                                        .avgPricePerM2(Math.round(unitAvg(e.getValue())))
                                        .build())
                .sorted(Comparator.comparingLong(LandStatsDto.Bucket::getAvgPricePerM2).reversed())
                .collect(Collectors.toList());
    }

    private String buildLandPrompt(
            String regionName,
            LandStatsDto stats,
            List<LandDealDto> deals,
            List<RealEstateNewsDto> news) {
        ...
        for (LandStatsDto.Bucket b : stats.getJimokBuckets()) {
            jimokBlock
                    ...
                    .append(b.getCount())
                    .append("건, 평균 ")
                    .append(unit(b.getAvgPricePerM2()))
                    .append("\n");
        }
        ...
        for (LandDealDto d : deals.stream().limit(10).collect(Collectors.toList())) {
            dealBlock
                    .append(i++)
                    .append(". ")
                    .append(Texts.nullSafe(d.getDong()))
                    .append(" ")
                    .append(Texts.nullSafe(d.getJimok()))
                    .append(" / ")
                    .append(Texts.nullSafe(d.getUseZone()))
                    .append(" — ")
                    .append(d.getAreaM2())
                    .append("㎡, ")
                    .append(money(d.getDealAmount()))
                    .append(" (단가 ")
                    .append(unit(d.getPricePerM2()))
                    .append(", ")
                    .append(d.getDealDate())
                    .append(")\n");
        }
        ...
        if (news.isEmpty()) {
            ...
            for (RealEstateNewsDto a : news) {
                newsBlock
                        .append(n++)
                        .append(". ")
                        .append(Texts.nullSafe(a.getTitle()))
                        .append("\n");
            }
    ...
    private RealEstateStatsDto computeStats(
            List<RealEstateDealDto> deals, String dealType, String sigungu) {
        ...
        long avg = prices.length == 0 ? 0 : Math.round(Arrays.stream(prices).average().orElse(0));
        ...
        // 평형별 (전용면적 10㎡ 구간) 집계
        Map<Integer, List<RealEstateDealDto>> byBucket =
                deals.stream()
                        .filter(d -> d.getAreaM2() > 0)
                        .collect(
                                Collectors.groupingBy(
                                        d -> ((int) (d.getAreaM2() / 10)) * 10,
                                        TreeMap::new,
                                        Collectors.toList()));
        List<RealEstateStatsDto.AreaBucket> buckets =
                byBucket.entrySet().stream()
                        .map(
                                e ->
                                        RealEstateStatsDto.AreaBucket.builder()
    ...
    private RealEstateAnalysisResult parseResult(String text) {
        ...
        try {
            ...
            log.warn(
                    "[AI/RealEstate] JSON 파싱 실패: {} (head: {})",
                    e.getMessage(),
                    Texts.truncate(text, 200));
            return RealEstateAnalysisResult.builder()
                    .trend("보합")
                    .headline("")
                    .priceLevel("")
    ...
    private String money(long manwon) {
        ...
        if (rest > 0)
            sb.append(eok > 0 ? " " : "")
                    .append(String.format(Locale.KOREA, "%,d", rest))
                    .append("만");
