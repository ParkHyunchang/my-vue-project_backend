...
public class MenuPermissionService {
    ...
    @Autowired private MenuPermissionRepository menuPermissionRepository;
        ...
        return menuPermissionRepository.findAll().stream()
                .collect(
                        Collectors.groupingBy(
                                ...
                                Collectors.mapping(
                                        MenuPermission::getMenuPath, Collectors.toList())));
    }
    ...
    public void initializeDefaultPermissions() {
        ...
        defaults.put("GUEST", Arrays.asList("/", "/chat"));
        defaults.put(
                "USER",
                Arrays.asList(
                        "/",
        ...
        defaults.forEach(
                (roleName, menuPaths) -> {
                    List<String> existing =
                            menuPermissionRepository.findByRoleName(roleName).stream()
                                    .map(MenuPermission::getMenuPath)
                                    .collect(Collectors.toList());

                    List<MenuPermission> missing =
                            menuPaths.stream()
